...
import TicketReportTable from "./_component/TicketReportTable";
...
const Page = () => {
  const [page, setPage] = useState<Page<TicketReportData>>({
    content: [],
    totalPages: 0,
    size: 10,
    number: 0,
  ...
  async function fetchTicketReport() {
    try {
      const response = await HttpAuthInstance.get(
        `/api/lookAtMe/report/ticketList`,
        {
          params: {
            search: inputValue,
            page: currentPage - 1,
  ...
  async function handleApproval(uuid: string) {
    if (window.confirm("승인하시겠습니까?")) {
      try {
        const url = `/api/lookAtMe/report/ticket/approve/${uuid}`;
        await HttpAuthInstance.patch(url, {});
        alert("식사권 신고가 승인되었습니다.");
        fetchTicketReport();
      } catch (error) {
        console.error("오류 발생", error);
      }
  ...
  async function handleRefusal(uuid: string) {
    if (window.confirm("반려하시겠습니까?")) {
      try {
        const url = `/api/lookAtMe/report/ticket/refusal/${uuid}`;
        await HttpAuthInstance.patch(url, {});
        alert("식사권 신고가 반려되었습니다.");
        fetchTicketReport();
      } catch (error) {
        console.error("오류 발생", error);
      }
