...
import TicketListTable from "./_component/TicketListTable";
...
const Page = () => {
  const [page, setPage] = useState<Page<TicketListData>>({
    content: [],
    totalPages: 0,
    size: 10,
    number: 0,
  ...
  async function fetchTicketList() {
    try {
      const response = await HttpAuthInstance.get(`/api/lookAtMe/ticketList`, {
        params: {
          search: inputValue,
          page: currentPage - 1,
        },
  ...
  async function handleApproval(uuid: string) {
    if (window.confirm("닫으시겠습니까?")) {
      try {
        const url = `/api/lookAtMe/ticket/changeState/${uuid}`;
