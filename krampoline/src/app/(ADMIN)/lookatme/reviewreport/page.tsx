...
import ReviewReportTable from "./_component/ReviewReportTable";
...
const Page = () => {
  const [page, setPage] = useState<Page<ReviewReportData>>({
    content: [],
    totalPages: 0,
    size: 10,
    number: 0,
  ...
  async function fetchReviewReport() {
    try {
      const response = await HttpAuthInstance.get(
        `/api/lookAtMe/report/reviewList`,
        {
          params: {
            search: inputValue,
            page: currentPage - 1,
  ...
  async function handleApproval(uuid: string) {
    if (window.confirm("승인하시겠습니까?")) {
      try {
        const url = `/api/lookAtMe/report/review/approve/${uuid}`;
        await HttpAuthInstance.patch(url, {});
        alert("리뷰 신고가 승인되었습니다.");
        fetchReviewReport();
      } catch (error) {
        console.error("오류 발생", error);
      }
  ...
  async function handleRefusal(uuid: string) {
    if (window.confirm("반려하시겠습니까?")) {
      try {
        const url = `/api/lookAtMe/report/review/refusal/${uuid}`;
        await HttpAuthInstance.patch(url, {});
        alert("리뷰 신고가 반려되었습니다.");
        fetchReviewReport();
      } catch (error) {
        console.error("오류 발생", error);
      }
