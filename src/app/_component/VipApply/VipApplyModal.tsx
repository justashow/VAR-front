...
const VipApplyModal = ({ closeModal }: { closeModal: () => void }) => {
  ...
  return (
    <div className="VipApplyContainer">
      <div className="VipApplyTitle">
        <h1> 안녕하세요,{user.nickname} 님 </h1>
        <div> VIP 신청을 위해서는 아래 정보가 필요해요. </div>
      </div>
      <form onSubmit={handleSubmit} className="VipApplyTitleContainer">
        <div>
          <div className="VipApplyTitle">VAR 계정</div>
          <input placeholder={user.email} disabled />
        </div>
        <div>
          <div className="VipApplyTitle">연락 받을 이메일</div>
          <input placeholder="자주 사용하는 이메일을 입력해주세요." />
        </div>
        <div>
          <div className="VipApplyTitle">VIP의 실명 또는 사업체명</div>
          <div className="VipApplySubTitle">
        ...
        <div>
          <div className="VipApplyTitle">연락처</div>
          <input type="number" placeholder="000-0000-0000" />
        </div>

        <div>
          <div className="VipApplyTitle">희망분야</div>
          <div className="VipApplySubTitle">
            VIP의 직무와 연관된 분야를 골라주세요
          </div>
          <article className="VipApply-Group">
            {reasons.map((reason) => (
              <button
                key={reason}
                type="button"
        ...
        <div className="input-group">
          <div className="VipApplyTitle">나를 소개하는 글</div>
          <div className="VipApplySubTitle">
        ...
        <div>
          <div className="VipApplyTitle">자신을 표현할 수 있는 링크</div>
